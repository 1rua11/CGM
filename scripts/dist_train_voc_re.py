...
from cgm.model_attn_aff_re import CGM
...
def validate(model=None, data_loader=None, cfg=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader),
            ...
            cls, segs, _, attn_pred, loss_ce = model(inputs,labels=cls_label)

            cls_pred = (cls>0).type(torch.int16)
            _f1 = evaluate.multilabel_score(cls_label.cpu().numpy()[0], cls_pred.cpu().numpy()[0])
            avg_meter.add({"cls_score": _f1, "ce_score": loss_ce})
...
def train(cfg):
    ...
    cgm = CGM(backbone=cfg.backbone.config,
    ...
    logging.info('\nNetwork config: \n%s'%(cgm))
    param_groups = cgm.get_param_groups()
    ...
    logging.info('\nOptimizer: \n%s' % optimizer)
    cgm = DistributedDataParallel(cgm, device_ids=[args.local_rank], find_unused_parameters=True)
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        cls, segs, attns, attn_pred, loss_ce = cgm(inputs, labels=cls_labels, seg_detach=args.seg_detach)

        cams, aff_mat = multi_scale_cam_with_aff_mat(cgm, inputs=inputs, scales=cfg.cam.scales)
        ...
        if n_iter <= cfg.train.cam_iters:
            loss = 1.0 * cls_loss + 1e-5*(seg_loss + aff_loss + loss_ce)# + 0.0 * reg_loss
        else: 
            loss = 1.0 * cls_loss + 0.1 * seg_loss + 0.1 * aff_loss + 0.005*loss_ce #+ 0.005 * reg_loss

        avg_meter.add({'cls_loss': cls_loss.item(), 'loss_ce': loss_ce.item(), 'seg_loss': seg_loss.item(), 'aff_loss': aff_loss.item()})
        ...
        if (n_iter+1) % cfg.train.log_iters == 0:
            ...
            if args.local_rank==0:   
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, loss_ce: %.4f, aff_loss: %.4f, pseudo_seg_loss %.4f, pseudo_seg_mAcc: %.4f"%(n_iter+1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('loss_ce'), avg_meter.pop('aff_loss'), avg_meter.pop('seg_loss'), seg_mAcc))
        ...
        if (n_iter+1) % cfg.train.eval_iters == 0:
            ckpt_name = os.path.join(cfg.work_dir.ckpt_dir, "cgm_iter_%d.pth"%(n_iter+1))
            if args.local_rank==0:
                logging.info('Validating...')
                torch.save(cgm.state_dict(), ckpt_name)
            cls_score, ce_score, seg_score, cam_score, aff_score = validate(model=cgm, data_loader=val_loader, cfg=cfg)
            if args.local_rank==0:
                logging.info("val cls score: %.6f"%(cls_score))
                logging.info("val ce score: %.6f"%(ce_score))
